...
class Mars_ECCV_Mask(object):
    ...
    def __init__(self, root='/data/datasets/', min_seq_len=0):
        self.root = osp.join(root, 'MARS-ECCV-mask')
        self.train_name_path = osp.join(self.root, 'info/train_name.txt')
        self.test_name_path = osp.join(self.root, 'info/test_name.txt')
        self.track_train_info_path = osp.join(self.root, 'info/tracks_train_info.mat')
        self.track_test_info_path = osp.join(self.root, 'info/tracks_test_info.mat')
    ...
    def _process_data(self, names, meta_data, home_dir=None, relabel=False, min_seq_len=0):
        ...
        for tracklet_idx in range(num_tracklets):
            ...
            # append image names with directory information
            img_paths = [osp.join(self.root, home_dir, img_name[:4], img_name.replace('.jpg', '.png')) for img_name in img_names]
            if len(img_paths) >= min_seq_len:
                img_paths = tuple(img_paths)
                tracklets.append((img_paths, pid, camid))
                num_imgs_per_tracklet.append(len(img_paths))
...
class iLIDSVID(object):
    ...
    def _prepare_split(self):
        if not osp.exists(self.split_path):
            ...
            num_ids_each = num_total_ids//2  # zhaoyang  num_ids_each = num_total_ids/2
...
class iLIDSVID_ECCV_Mask(object):
    """
    iLIDS-VID
    ...
    def __init__(self, root='/data/datasets/', split_id=9):
        self.root = osp.join(root, 'iLIDS-VID-ECCV-mask')
        # self.dataset_url = 'http://www.eecs.qmul.ac.uk/~xiatian/iLIDS-VID/iLIDS-VID.tar'
        self.data_dir = osp.join(self.root, 'i-LIDS-VID')
        self.split_dir = osp.join(self.root, 'train-test people splits')
        self.split_mat_path = osp.join(self.split_dir, 'train_test_splits_ilidsvid.mat')
        ...
        print("=> iLIDS-VID-ECCV-mask loaded")
        print("Dataset statistics:")
        print("  ------------------------------")
        print("  subset   | # ids | # tracklets")
        print("  ------------------------------")
...
class DukeMTMCVidReID_ECCV_Mask(object):
    """
    DukeMTMCVidReID
    Reference:
    Wu et al. Exploit the Unknown Gradually: One-Shot Video-Based Person
    ...
    def __init__(self, root='/data/datasets/', min_seq_len=0, verbose=True, **kwargs):
        self.dataset_dir = osp.join(root, 'DukeMTMC-VideoReID-ECCV-mask')
        self.dataset_url = 'http://vision.cs.duke.edu/DukeMTMC/data/misc/DukeMTMC-VideoReID.zip'
        self.train_dir = osp.join(self.dataset_dir, 'train')
        self.query_dir = osp.join(self.dataset_dir, 'query')
        self.gallery_dir = osp.join(self.dataset_dir, 'gallery')
        ...
        if verbose:
            print("=> DukeMTMC-VideoReID-ECCV-mask loaded")
            print("Dataset statistics:")
            print("  ------------------------------")
            print("  subset   | # ids | # tracklets")
            print("  ------------------------------")
        ...
        for i in range(len(self.train)):
            for j in range(len(self.train[i][0])):
                self.train[i][0][j] = self.train[i][0][j].replace('.jpg', '.png')
                self.train[i][0][j] = self.train[i][0][j].replace('DukeMTMC-VideoReID', 'DukeMTMC-VideoReID-ECCV-mask')

        for i in range(len(self.train_dense)):
            for j in range(len(self.train_dense[i][0])):
                self.train_dense[i][0][j] = self.train_dense[i][0][j].replace('.jpg', '.png')
                self.train_dense[i][0][j] = self.train_dense[i][0][j].replace('DukeMTMC-VideoReID', 'DukeMTMC-VideoReID-ECCV-mask')

        for i in range(len(self.query)):
            for j in range(len(self.query[i][0])):
                self.query[i][0][j] = self.query[i][0][j].replace('.jpg', '.png')
                self.query[i][0][j] = self.query[i][0][j].replace('DukeMTMC-VideoReID', 'DukeMTMC-VideoReID-ECCV-mask')

        for i in range(len(self.gallery)):
            for j in range(len(self.gallery[i][0])):
                self.gallery[i][0][j] = self.gallery[i][0][j].replace('.jpg', '.png')
                self.gallery[i][0][j] = self.gallery[i][0][j].replace('DukeMTMC-VideoReID', 'DukeMTMC-VideoReID-ECCV-mask')
...
__factory = {
    ...
    'mars_eccv_mask': Mars_ECCV_Mask,
    'ilidsvid_eccv_mask': iLIDSVID_ECCV_Mask,
    'duke_eccv_mask': DukeMTMCVidReID_ECCV_Mask,
}
