...
parser = argparse.ArgumentParser(description='Test wo KD using all frames')
# Datasets
...
# Architecture
parser.add_argument('-a', '--arch', type=str, default='resnet50tp_2branch',
...
def main():
    ...
    for epoch in args.test_epochs:
        model_path = osp.join(args.resume, 'checkpoint.pth.tar')
...
def extract(model, vids,vids_gait, use_gpu):
    n, c, f, h, w = vids.size()
    assert(n == 1)

    if use_gpu:
        feat_a = torch.cuda.FloatTensor()
        feat_g = torch.cuda.FloatTensor()
    else:
        feat_a = torch.FloatTensor()
        feat_g = torch.FloatTensor()
    for i in range(math.ceil(f/args.test_frames)):
        clip = vids[:, :, i*args.test_frames:(i+1)*args.test_frames, :, :]
        clip_gait = vids_gait[:, :, i * args.test_frames:(i + 1) * args.test_frames, :, :]

        if use_gpu:
            clip = clip.cuda()
            clip_gait = clip_gait.cuda()

        output_a, output_g = model(clip,clip_gait)
        feat_a = torch.cat((feat_a, output_a), 2)
        feat_g = torch.cat((feat_g, output_g), 2)

    feat_a = F.avg_pool1d(feat_a, feat_a.size(2))
    feat_g = F.avg_pool1d(feat_g, feat_g.size(2))

    f_cat = torch.cat((feat_a, feat_g), 2).contiguous().unsqueeze(3)

    att = model.attconv(f_cat)
    ...
    feat = model.groupingbn(feat)
    feat = model.nonlinear(feat)
...
def test(model, queryloader, galleryloader, use_gpu):
    ...
    for batch_idx, (vids,vids_gait, pids, camids) in enumerate(queryloader):
        ...
        qf.append(extract(model, vids,vids_gait, use_gpu).squeeze())
        q_pids.extend(pids)
    ...
    for batch_idx, (vids,vids_gait, pids, camids) in enumerate(galleryloader):
        ...
        gf.append(extract(model, vids,vids_gait, use_gpu).squeeze())
        g_pids.extend(pids)
...
# python test_baseline-all.py --root /home/yang/Documents/DataSet/person_reid_video/ -d ilidsvid --arch resnet50tp_2branch --gpu 0 --resume log-mars-ap3d-debug --test_epochs 1
